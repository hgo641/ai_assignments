...
def get_new_weight2(weight_jk, diff):
    weight_jk = np.array(weight_jk)
    new_weights = []
    for i in range(3):
        temp = []
...
def get_contribution(error, output, output_j):
    diff = []
    for i in range(3):
        # E - a20
        err = error[i]
...
def backward(target, actual, second_output, layer2_weights, input_user, layer1_weights):
    ...
    diff = get_contribution(e_output, actual, second_output)
    new_layer2_weights = get_new_weight2(layer2_weights, diff)
...
if __name__ == '__main__':
    second_output = matrix_mul(input_user, layer1_weights)  # output j
    final_output = matrix_mul(second_output, layer2_weights)  # output k
    ...
    print(get_diff(e_output, layer2_weights, second_output))
    new_layer_2_weights = get_new_weight(layer2_weights, diff)
    print(get_new_weight(layer2_weights, diff))
    print("____blog.ver_____")
    diff2 = get_contribution(e_output, actual, second_output)
    print(get_contribution(e_output, actual, second_output))
    new_layer_2_weights2 = get_new_weight2(layer2_weights, diff)
    print(get_new_weight2(layer2_weights, diff))
    print("--------layer1 .... ------")
    print(get_diff_layer1(e_output, actual, layer2_weights, second_output, input_user))
    diff_layer1 = get_diff_layer1(e_output, actual, layer2_weights, second_output, input_user)
    print(get_new_weight2(layer1_weights, diff_layer1))
    print(get_new_weight(layer1_weights, diff_layer1))

    print("2차 forward")
    new_layer2_weights = get_new_weight2(layer2_weights, diff)
    new_layer1_weights = get_new_weight(layer1_weights, diff_layer1)

    second_output2 = matrix_mul(input_user, new_layer1_weights)  # output j
    final_output2 = matrix_mul(second_output2, new_layer2_weights)  # output k

    actual = []
    for i in final_output2:
        string = str(i)
        num = ''
        for j in range(5):
            num += string[j]
